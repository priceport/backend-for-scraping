...
const champagne_and_sparkling = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.com.au/wine/champagne-and-sparkling?page=';
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        productElements.forEach(product => {
          ...
          if(!title||!brand||!price||!url){}
              ...
              last_check: Date.now(),
              unit: undefined,
              subcategory: 'champagne_and_sparkling',
              img
            });
        });
      ...
      if (missing > 5) {
        await insertScrapingError(`More than 5 entries missing for liquorland_aus - wine-champagne-and-sparkling: ${pageNo}`, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError(`Error in liquorland_aus - wine-champagne-and-sparkling: ${err.message}`, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
...
module.exports = champagne_and_sparkling;