...
const tequila = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/wine-and-spirits/tequila?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Tequila] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - tequila: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - tequila: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
