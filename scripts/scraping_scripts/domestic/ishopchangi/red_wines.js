...
const red_wines = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/wine-and-spirits/wines-and-champagnes/red-wines?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Red Wines] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - red_wines: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - red_wines: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
