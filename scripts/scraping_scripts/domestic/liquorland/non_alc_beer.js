...
const non_alc_beer = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/beer/non-alc-beer?p=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - non_alc_beer: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - non_alc_beer: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
