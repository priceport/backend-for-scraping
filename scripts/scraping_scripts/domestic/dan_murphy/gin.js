...
const gin = async () => {
    ...
    console.log("Navigating to Dan Murphy's gin page...");
    ...
    // Then navigate to the gin page
    retryCount = 0;
    let ginPageLoaded = false;
    
    while (retryCount < maxRetries && !ginPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/gin', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Gin page loaded successfully");
            ginPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading gin page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/gin', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Gin page loaded with domcontentloaded");
                    ginPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load gin page after all retries");
                throw new Error(`Failed to load gin page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/gin?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
