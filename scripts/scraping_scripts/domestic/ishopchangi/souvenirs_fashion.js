...
const souvenirs_fashion = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/souvenirs/souvenirs-fashion?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Souvenirs Fashion] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - souvenirs_fashion: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - souvenirs_fashion: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = souvenirs_fashion;
