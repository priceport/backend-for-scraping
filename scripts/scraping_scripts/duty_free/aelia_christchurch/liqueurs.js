...
const liqueurs = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://www.aeliadutyfree.co.nz/christchurch/spirits/liqueurs.html?p=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for aelia_christchurch - liqueurs: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in aelia_christchurch - liqueurs: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
