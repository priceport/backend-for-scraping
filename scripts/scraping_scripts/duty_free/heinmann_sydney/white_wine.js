...
const white_wine = async (start,end,browser)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
            await insertScrapingError("More than 5 entries missing for heinemann_sydney - white_wine: "+pageNo,"scraping_missing");
        }
        ...
        pageNo+=1;
            ...
            logError(err);
            try{
                await insertScrapingError("Error in heinemann_sydney - white_wine: "+err.message,"scraping_trycatch");
              }catch(err){
                  console.log(err);
              }
            await page.close();
