...
const grooming = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers grooming scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" grooming ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - grooming: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers grooming scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - grooming: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
