...
const premium_spirits = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://www.aeliadutyfree.co.nz/auckland/spirits/premium-spirits.html?p=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for aelia_auckland - premium_spirits: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in aelia_auckland - premium_spirits: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
