...
const sun_care = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/bath-body-care/suncare-tanning';
    const url = 'https://www.farmers.co.nz/beauty/bath-body-care/suncare-tanning/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - sun_care: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - sun_care: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
