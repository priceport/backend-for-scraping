const bath = require("../scripts/scraping_scripts/domestic/au_sephora/bath");
const clean = require("../scripts/scraping_scripts/domestic/au_sephora/clean");
const fragrance = require("../scripts/scraping_scripts/domestic/au_sephora/fragrance");
const hair = require("../scripts/scraping_scripts/domestic/au_sephora/hair");
const makeup = require("../scripts/scraping_scripts/domestic/au_sephora/makeup");
const skincare = require("../scripts/scraping_scripts/domestic/au_sephora/skincare");
const tools = require("../scripts/scraping_scripts/domestic/au_sephora/tools");
const processDataForBeauty = require("./data_processing/au_sephora/beauty");
const updateDBEntry = require("./update_db_entry/au_sephora/beauty");

const logError = require("./logError");

const scrapeAuSephora = async (start, end, state, browser) => {
  console.log("scraping started for sephora at:" + Date.now());

  //variable initialization
  let makeupData = [],
    skincareData = [],
    toolsData = [],
    hairData = [],
    fragranceData = [],
    cleanData = [],
    bathData = [];

  //scrape each category
  if (!state.auSephora.makeup)
    try {
      makeupData = await makeup(start, end, browser);
      console.log(`${makeupData?.length} data items scraped for makeup`);
    } catch (err) {
      console.log("There was an error while scraping makeup");
      logError(err);
    }

  if (!state.auSephora.makeup && makeupData?.length == 0)
    try {
      makeupData = await makeup(start, end, browser);
      console.log(`${makeupData?.length} data items scraped for makeup`);

      if (makeupData?.length == 0) {
        state.auSephora.makeup = true;
      }
    } catch (err) {
      console.log("There was an error while scraping makeup");
      logError(err);
    }

  if (!state.auSephora.skincare)
    try {
      skincareData = await skincare(start, end, browser);
      console.log(`${skincareData?.length} data items scraped for skincare`);
    } catch (err) {
      console.log("There was an error while scraping skincare");
      logError(err);
    }

  if (!state.auSephora.skincare && skincareData?.length == 0)
    try {
      skincareData = await skincare(start, end, browser);
      console.log(`${skincareData?.length} data items scraped for skincare`);

      if (skincareData?.length == 0) {
        state.auSephora.skincare = true;
      }
    } catch (err) {
      console.log("There was an error while scraping skincare");
      logError(err);
    }

  if (!state.auSephora.tools)
    try {
      toolsData = await tools(start, end, browser);
      console.log(`${toolsData?.length} data items scraped for tools`);
    } catch (err) {
      console.log("There was an error while scraping tools");
      logError(err);
    }

  if (!state.auSephora.tools && toolsData?.length == 0)
    try {
      toolsData = await tools(start, end, browser);
      console.log(`${toolsData?.length} data items scraped for tools`);

      if (toolsData?.length == 0) {
        state.auSephora.tools = true;
      }
    } catch (err) {
      console.log("There was an error while scraping tools");
      logError(err);
    }

  if (!state.auSephora.hair)
    try {
      hairData = await hair(start, end, browser);
      console.log(`${hairData?.length} data items scraped for hair`);
    } catch (err) {
      console.log("There was an error while scraping hair");
      logError(err);
    }

  if (!state.auSephora.hair && hairData?.length == 0)
    try {
      hairData = await hair(start, end, browser);
      console.log(`${hairData?.length} data items scraped for hair`);

      if (hairData?.length == 0) {
        state.auSephora.hair = true;
      }
    } catch (err) {
      console.log("There was an error while scraping hair");
      logError(err);
    }

  if (!state.auSephora.fragrance)
    try {
      fragranceData = await fragrance(start, end, browser);
      console.log(`${fragranceData?.length} data items scraped for fragrance`);
    } catch (err) {
      console.log("There was an error while scraping fragrance");
      logError(err);
    }

  if (!state.auSephora.fragrance && fragranceData?.length == 0)
    try {
      fragranceData = await fragrance(start, end, browser);
      console.log(`${fragranceData?.length} data items scraped for fragrance`);

      if (fragranceData?.length == 0) {
        state.auSephora.fragrance = true;
      }
    } catch (err) {
      console.log("There was an error while scraping fragrance");
      logError(err);
    }

  if (!state.auSephora.clean)
    try {
      cleanData = await clean(start, end, browser);
      console.log(`${cleanData?.length} data items scraped for clean`);
    } catch (err) {
      console.log("There was an error while scraping clean");
      logError(err);
    }

  if (!state.auSephora.clean && cleanData?.length == 0)
    try {
      cleanData = await clean(start, end, browser);
      console.log(`${cleanData?.length} data items scraped for clean`);

      if (cleanData?.length == 0) {
        state.auSephora.clean = true;
      }
    } catch (err) {
      console.log("There was an error while scraping clean");
      logError(err);
    }

  if (!state.auSephora.bath)
    try {
      bathData = await bath(start, end, browser);
      console.log(`${bathData?.length} data items scraped for bath`);
    } catch (err) {
      console.log("There was an error while scraping bath");
      logError(err);
    }

  if (!state.auSephora.bath && bathData?.length == 0)
    try {
      bathData = await bath(start, end, browser);
      console.log(`${bathData?.length} data items scraped for bath`);

      if (bathData?.length == 0) {
        state.auSephora.bath = true;
      }
    } catch (err) {
      console.log("There was an error while scraping bath");
      logError(err);
    }

  //merge data
  makeupData = [
    ...makeupData,
    ...skincareData,
    ...toolsData,
    ...hairData,
    ...fragranceData,
    ...cleanData,
    ...bathData,
  ];

  //process data
  try {
    makeupData = await processDataForBeauty(makeupData);
    console.log(`${makeupData?.length} data items proccessed`);
  } catch (err) {
    console.log("There was an error while proccessing data");
    logError(err);
  }
  //update db
  try {
    await updateDBEntry(makeupData);
    console.log(`data items updated`);
  } catch (err) {
    console.log("There was an error while updating data");
    logError(err);
  }

  console.log("entries updated for au_sephora");
  return makeupData?.length == 0;
};

module.exports = scrapeAuSephora;
