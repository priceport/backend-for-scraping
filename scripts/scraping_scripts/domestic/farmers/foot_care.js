...
const foot_care = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers foot_care scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" foot_care ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    console.log(`Farmers foot_care scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - foot_care: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
