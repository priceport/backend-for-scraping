...
const mens_apparels = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/mens-fashion/mens-apparels?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Men's Apparels] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - mens_apparels: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - mens_apparels: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
