...
const serum = async (start,end,browser)=>{
    let pageNo = start-1;
    const url = "https://www.heinemann.com.au/en/sydt1/skin-care/skin-care/serum/c/auscat_2013/?q=%3Arelevance&page=";
    ...
    while(true){
        ...
        if(missing > 5) {
            await insertScrapingError("More than 5 entries missing for heinemann_sydney - serum: "+pageNo,"scraping_missing");
        }
        ...
        pageNo+=1;
            ...
            logError(err);
            try{
                await insertScrapingError("Error in heinemann_sydney - serum: "+err.message,"scraping_trycatch");
              }catch(err){
                  console.log(err);
              }
            await page.close();
