...
const flavoured_and_sours = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.com.au/beer-and-cider/beer/flavoured-and-sours?page=';
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        productElements.forEach(product => {
          ...
          if(!title||!brand||!price||!url){}
              ...
              last_check: Date.now(),
              unit: undefined,
              subcategory: 'flavoured_and_sours',
              img
            });
        });
      ...
      if (missing > 5) {
        await insertScrapingError(`More than 5 entries missing for liquorland_aus - beer-and-cider-beer-flavoured-and-sours: ${pageNo}`, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError(`Error in liquorland_aus - beer-and-cider-beer-flavoured-and-sours: ${err.message}`, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
...
module.exports = flavoured_and_sours;