...
const gromming = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/skincare/men-s-skincare-grooming';
    const url = 'https://www.farmers.co.nz/beauty/skincare/men-s-skincare-grooming/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - gromming: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - gromming: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
