...
const beauty_tools = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/home-and-living/beauty-tools?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Beauty Tools] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - beauty_tools: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - beauty_tools: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
