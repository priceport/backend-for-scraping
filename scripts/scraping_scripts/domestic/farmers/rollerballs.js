...
const rollerballs = async (start, end, browser, sharedPage) => {
  ...
  const allProducts = [];
  let missingTotal = 0;
  // Note: Rollerballs cannot be scraped directly due to robots.txt restrictions
  // /filter/* and SearchTerm= are disallowed. Using perfume category instead.
  const baseUrl = "https://www.farmers.co.nz/beauty/perfume";
  ...
  try {
    ...
    console.log(`✅ rollerballs scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ rollerballs scraping completed: ${formattedProducts.length} products`);
