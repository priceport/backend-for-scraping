/**
 * Farmers Hair Care Products Scraper
...
const hair_care = async (start, end) => {
  ...
  try {
    console.log(`Starting Farmers hair_care scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" hair_care ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    console.log(`Farmers hair_care scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - hair_care: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
