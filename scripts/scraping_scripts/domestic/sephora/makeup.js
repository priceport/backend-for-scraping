...
const makeup = async (start,end)=>{
    let pageNo = start;
    const url = 'https://www.sephora.nz/categories/makeup?page=replace_me&view=120';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for sephora - makeup: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in sephora - makeup: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
