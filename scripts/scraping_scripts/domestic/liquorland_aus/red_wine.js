...
const red_wine = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.com.au/wine/red-wine?page=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(`More than 5 entries missing for liquorland_aus - wine-red-wine: ${pageNo}`, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError(`Error in liquorland_aus - wine-red-wine: ${err.message}`, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
