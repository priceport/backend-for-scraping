...
const body_care = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers body_care scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" body_care ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - body_care: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers body_care scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - body_care: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
