...
const eye_cream = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/skincare/eye-cream';
    const url = 'https://www.farmers.co.nz/beauty/skincare/eye-cream/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for farmers - eye_cream: "+pageNo,"scraping_missing");
        }
        ...
        try{
          await insertScrapingError("Error in farmers - eye_cream: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
