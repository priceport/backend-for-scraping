...
const bath_care = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers bath_care scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" bath_care ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - bath_care: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers bath_care scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - bath_care: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
