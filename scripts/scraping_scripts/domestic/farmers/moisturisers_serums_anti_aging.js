...
const moisturisers_serums_anti_aging = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/skincare/moisturisers-serums-anti-aging";
  ...
  try {
    ...
    const formattedProducts = allProducts.map(product => {
      ...
      return {
        ...
        mapping_ref: null,
        unit: undefined,
        subcategory: 'moisturisers_serums_anti_aging',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - moisturisers_serums_anti_aging: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ moisturisers_serums_anti_aging scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ moisturisers_serums_anti_aging scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - moisturisers_serums_anti_aging (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = moisturisers_serums_anti_aging;
