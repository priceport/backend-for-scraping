...
const tools2 = async (start, end, browser) => {
  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/makeup/makeup-tools-accessories";
  ...
  try {
    ...
    try {
      await insertScrapingError(
        `Error in farmers - tools2 (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
