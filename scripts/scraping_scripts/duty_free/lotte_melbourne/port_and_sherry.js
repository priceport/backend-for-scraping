...
const port_and_sherry = async (start,end,browser)=>{
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const apiData = await fetchImageFromAPI(product.url, 'port-and-sherry');
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_melbourne - port-and-sherry: "+pageNo,"scraping_missing");
    }
