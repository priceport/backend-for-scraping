...
const cleansing = async (start,end,browser)=>{
    let pageNo = start-1;
    const url = "https://www.heinemann.com.au/en/ool/skin-care/skin-care/cleansing/c/cat_2012/?q=%3Arelevance&page=";
    ...
    while(true){
        ...
        if(missing > 5) {
            await insertScrapingError("More than 5 entries missing for heinemann_goldcoast - cleansing: "+pageNo,"scraping_missing");
        }
        ...
        pageNo+=1;
            ...
            logError(err);
            try{
                await insertScrapingError("Error in heinemann_goldcoast - cleansing: "+err.message,"scraping_trycatch");
              }catch(err){
                  console.log(err);
              }
            await page.close();
