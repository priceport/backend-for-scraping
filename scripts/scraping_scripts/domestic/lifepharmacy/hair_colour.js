...
const hair_colour = async (start, end, browser) => {
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy hair_colour] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy hair_colour] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy hair_colour page ${pageNo}`);
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy hair_colour] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy hair_colour] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - hair_colour: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
