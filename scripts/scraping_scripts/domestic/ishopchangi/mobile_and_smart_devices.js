...
const mobile_and_smart_devices = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/electronics/mobile-and-smart-devices?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Mobile & Smart Devices] Loading page ${pageNo}...`);
      ...
      const [products,missing] = await page.evaluate(() => {
        ...
        productElements.forEach((productLink, index) => {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "mobile_and_smart_devices",
              img: imgSrc,
            });
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - mobile_and_smart_devices: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - mobile_and_smart_devices: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = mobile_and_smart_devices;
