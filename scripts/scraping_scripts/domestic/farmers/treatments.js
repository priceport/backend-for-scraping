...
const treatments = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/skincare/treatments-masks';
    const url = 'https://www.farmers.co.nz/beauty/skincare/treatments-masks/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for farmers - treatments: "+pageNo,"scraping_missing");
        }
        ...
        try{
          await insertScrapingError("Error in farmers - treatments: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
