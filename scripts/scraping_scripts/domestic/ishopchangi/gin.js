...
const gin = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/wine-and-spirits/gin?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Gin] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - gin: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - gin: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
