...
const wellness_and_health_devices = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/home-and-living/wellness-and-health-devices?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Wellness and Health Devices] Loading page ${pageNo}...`);
      ...
      const [products,missing] = await page.evaluate(() => {
        ...
        productElements.forEach((productLink, index) => {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "wellness_and_health_devices",
              img: imgSrc,
            });
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - wellness_and_health_devices: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - wellness_and_health_devices: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = wellness_and_health_devices;
