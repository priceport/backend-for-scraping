...
const gift_sets = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://www.aucklanddutyfree.co.nz/beauty/gift-sets.html?p=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for aelia_auckland - gift_sets: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in aelia_auckland - gift_sets: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
