...
const eyes = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/makeup/eyes';
    const url = 'https://www.farmers.co.nz/beauty/makeup/eyes/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - eyes: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - eyes: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
