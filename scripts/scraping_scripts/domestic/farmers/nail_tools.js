...
const nail_tools = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/nail-care/nail-care-tools';
    const url = 'https://www.farmers.co.nz/beauty/nail-care/nail-care-tools/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - nail_tools: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - nail_tools: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
