...
const bath_care = async (start,end,browser)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for farmers - bath_care: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
            await insertScrapingError("Error in farmers - bath_care: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
