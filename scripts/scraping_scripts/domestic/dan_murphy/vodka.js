...
const vodka = async () => {
    ...
    retryCount = 0;
    let vodkaPageLoaded = false;
    
    while (retryCount < maxRetries && !vodkaPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/vodka', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Vodka page loaded successfully");
            vodkaPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading vodka page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/vodka', { waitUntil: 'domcontentloaded', timeout: 90000 });
                ...
                console.log("Failed to load vodka page after all retries");
                throw new Error(`Failed to load vodka page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/vodka?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
