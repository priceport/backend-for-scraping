...
const beard = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-grooming-beard/";
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - beard: " + pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - beard: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
