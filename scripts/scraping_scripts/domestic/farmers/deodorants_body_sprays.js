...
const deodorants_body_sprays = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/perfume/deodorants-body-sprays";
  ...
  try {
    ...
    const formattedProducts = allProducts.map(product => {
      ...
      return {
        ...
        mapping_ref: null,
        unit: undefined,
        subcategory: 'deodorants_body_sprays',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - deodorants_body_sprays: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ deodorants_body_sprays scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ deodorants_body_sprays scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - deodorants_body_sprays (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = deodorants_body_sprays;
