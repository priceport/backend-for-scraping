...
const baby_gears = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/babies-and-kids/babies-and-kids-baby-gears?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Baby Gears] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - baby_gears: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - baby_gears: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
