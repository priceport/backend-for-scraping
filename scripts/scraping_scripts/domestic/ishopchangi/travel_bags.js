...
const travel_bags = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/travel-and-services/travel-bags?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Travel Bags] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - travel_bags: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - travel_bags: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
