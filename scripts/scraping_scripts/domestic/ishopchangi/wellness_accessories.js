...
const wellness_accessories = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/health-and-wellness/wellness-accessories?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Wellness Accessories] Loading page ${pageNo}...`);
      ...
      const [products,missing] = await page.evaluate(() => {
        ...
        productElements.forEach((productLink, index) => {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "wellness_accessories",
              img: imgSrc,
            });
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - wellness_accessories: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - wellness_accessories: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = wellness_accessories;
