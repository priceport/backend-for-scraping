...
const port_and_sherry = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/wine/port-and-sherry?p=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - port_and_sherry: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - port_and_sherry: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
