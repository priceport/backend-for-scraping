...
const wellness_skincare = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/wellness-supplements/skincare';
    const url = 'https://www.farmers.co.nz/beauty/wellness-supplements/skincare/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - wellness_skincare: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - wellness_skincare: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
