...
const deodorants = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/perfume/deodorants-body-sprays';
    const url = 'https://www.farmers.co.nz/beauty/perfume/deodorants-body-sprays/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - deodorants: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - deodorants: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
