...
const international_beer = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/beer/international?p=';
  ...
  try {
    ...
    while (true) {
        ...
        productElements.forEach(product => {
          ...
          if (!title || !price || !url) {
            ...
            productList.push({ 
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: 'international_beer',
              img
            });
          }
        });
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - international_beer: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - international_beer: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
...
module.exports = international_beer;
