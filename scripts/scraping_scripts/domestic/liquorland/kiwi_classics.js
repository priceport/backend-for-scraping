...
const kiwi_classics = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/beer/kiwi-classics?p=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - kiwi_classics: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - kiwi_classics: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
