...
const champagneSparkling = async () => {
    ...
    console.log("Navigating to Dan Murphy's champagne sparkling page...");
    ...
    // Then navigate to the champagne sparkling page
    retryCount = 0;
    let champagnePageLoaded = false;
    
    while (retryCount < maxRetries && !champagnePageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/champagne-sparkling/all', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Champagne sparkling page loaded successfully");
            champagnePageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading champagne sparkling page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/champagne-sparkling/all', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Champagne sparkling page loaded with domcontentloaded");
                    champagnePageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load champagne sparkling page after all retries");
                throw new Error(`Failed to load champagne sparkling page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/champagne-sparkling/all?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
            ...
            try {
                ...
                if (name) {
                    products.push({
                        ...
                        category: 'liquor',
                        sub_category: 'champagne_sparkling',
                        website: 'danmurphy',
                        country: 'Australia',
                    });
                }
...
module.exports = champagneSparkling;