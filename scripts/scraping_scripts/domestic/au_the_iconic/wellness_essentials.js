...
const wellness_essentials = async (start, end, browser) => {
  let pageNo = start;
  const url =
    "https://www.theiconic.com.au/beauty-wellness-wellness-essentials/";
  ...
  try {
    ...
    while (true) {
        ...
        productElements.forEach((product) => {
          ...
          if (!title && !brand && !finalPrice && !promo && !fullUrl) {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "wellness_essentials",
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - wellness_essentials: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - wellness_essentials: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = wellness_essentials;
