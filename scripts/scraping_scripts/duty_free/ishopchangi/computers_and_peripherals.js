...
const computers_and_peripherals = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/electronics/computers-and-peripherals?page=";
  ...
  try {
      ...
      const [products,missing] = await page.evaluate(() => {
        ...
        productElements.forEach((productLink, index) => {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "computers_and_peripherals",
              img: imgSrc,
            });
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - computers_and_peripherals: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = computers_and_peripherals;
