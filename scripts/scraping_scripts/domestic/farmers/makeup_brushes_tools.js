...
const makeup_brushes_tools = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/makeup/makeup-brushes-tools";
  ...
  try {
    ...
    const formattedProducts = allProducts.map(product => {
      ...
      return {
        ...
        mapping_ref: null,
        unit: undefined,
        subcategory: 'makeup_brushes_tools',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - makeup_brushes_tools: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ makeup_brushes_tools scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ makeup_brushes_tools scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - makeup_brushes_tools (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = makeup_brushes_tools;
