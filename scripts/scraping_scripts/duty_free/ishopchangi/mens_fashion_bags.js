...
const mens_fashion_bags = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/mens-fashion/mens-fashion-bags?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Men's Fashion Bags] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - mens_fashion_bags: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - mens_fashion_bags: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = mens_fashion_bags;
