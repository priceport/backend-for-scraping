...
const toys_and_games = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/babies-and-kids/babies-and-kids-toys-and-games?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Toys and Games] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - toys_and_games: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - toys_and_games: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
