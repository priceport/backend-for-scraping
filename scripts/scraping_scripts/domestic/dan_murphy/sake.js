...
const sake = async () => {
    ...
    console.log("Navigating to Dan Murphy's sake page...");
    ...
    // Then navigate to the sake page
    retryCount = 0;
    let sakePageLoaded = false;
    
    while (retryCount < maxRetries && !sakePageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/sake', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Sake page loaded successfully");
            sakePageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading sake page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/sake', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Sake page loaded with domcontentloaded");
                    sakePageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load sake page after all retries");
                throw new Error(`Failed to load sake page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/sake?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
