...
const cocktail_essentials = async (start, end, browser) => {
  let pageNo = start;
  const baseUrl = 'https://www.liquorland.co.nz/shop/liqueurs/cocktail-essentials?FF=&InStoreOnly=false&P.StartPage=';
  ...
  try {
    ...
    while (true) {
          ...
          try {
            ...
            if (title && price) {
              ...
              productList.push({
                ...
                mapping_ref: null,
                unit: undefined,
                subcategory: 'cocktail_essentials',
                img
              });
            }
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - cocktail_essentials: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - cocktail_essentials: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
...
module.exports = cocktail_essentials;
