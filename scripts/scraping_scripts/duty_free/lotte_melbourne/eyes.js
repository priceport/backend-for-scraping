...
const eyes = async (start,end,browser)=>{
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const apiData = await fetchImageFromAPI(product.url, 'eyes');
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_melbourne - eyes: "+pageNo,"scraping_missing");
    }
