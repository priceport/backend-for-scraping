...
const tools2 = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/makeup/makeup-brushes-tools';
    const url = 'https://www.farmers.co.nz/beauty/makeup/makeup-brushes-tools/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - tools2: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - tools2: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
