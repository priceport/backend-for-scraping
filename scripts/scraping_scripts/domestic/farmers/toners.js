...
const toners = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers toners scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" toners ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - toners: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers toners scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - toners: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
