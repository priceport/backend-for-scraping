...
const bath_shower_care = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/bath-body-care/bath-shower-care";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - bath_shower_care: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ bath_shower_care scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ bath_shower_care scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - bath_shower_care (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = bath_shower_care;
