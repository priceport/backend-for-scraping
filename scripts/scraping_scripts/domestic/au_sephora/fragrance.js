...
const fragrance = async (start, end) => {
  let pageNo = start;
  const url =
    "https://www.sephora.com.au/categories/fragrance?page=replace_me&view=120";
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for au_sephora - fragrance: " + pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in au_sephora - fragrance: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
