/**
 * Farmers Hair Color Products Scraper
...
const hair_color = async (start, end) => {
  const allProducts = [];
  ...
  try {
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" hair_color ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    console.log(`Farmers hair_color scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - hair_color: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
