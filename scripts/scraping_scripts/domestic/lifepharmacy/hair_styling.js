...
const hair_styling = async (start, end, browser) => {
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy hair_styling] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy hair_styling] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy hair_styling page ${pageNo}`);
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy hair_styling] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy hair_styling] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - hair_styling: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
