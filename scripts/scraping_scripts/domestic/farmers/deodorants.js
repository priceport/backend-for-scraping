/**
 * Farmers Deodorants Products Scraper
...
const deodorants = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers deodorants scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" deodorants ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    console.log(`Farmers deodorants scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - deodorants: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
