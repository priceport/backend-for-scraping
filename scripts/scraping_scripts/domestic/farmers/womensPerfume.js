...
const womensPerfume = async (start, end, browser) => {
  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/fragrance/womens-perfume";
  ...
  try {
    ...
    try {
      await insertScrapingError(
        `Error in farmers - womensPerfume (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
