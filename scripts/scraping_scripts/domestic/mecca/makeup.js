...
const makeup = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://www.mecca.com/en-nz/makeup/?page=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for mecca - makeup: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in farmers - makeup: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
