...
const treatments_masks = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/skincare/treatments-masks";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - treatments_masks: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ treatments_masks scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ treatments_masks scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - treatments_masks (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = treatments_masks;
