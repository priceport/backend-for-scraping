        ...
        try {
            ...
            if (product.rowCount === 0) {
                ...
                product = await pool.query(
                    ...
                    [title, brand, "No desc", url, img, quantity, unit, category, "lotte_melbourne", "duty-free",sub_category]
                );
            } else {
            ...
            if (latestPrice.rowCount === 0 || latestPrice.rows[0].price != price[0].price.toFixed(3)) {
                await pool.query(
                    ...
                    [product?.rows[0]?.id, price[0].price, "lotte_melbourne", price_per_unit]
                );
                new_prices += 1;
            }
            ...
            if (promo) {
                for (let i = 0; i < promo?.length; i++) {
                    await pool.query(
                        ...
                        [product?.rows[0]?.id, promo[i]?.text, promo[i]?.price, "lotte_melbourne"]
                    );
                }
            }
