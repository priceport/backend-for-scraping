...
const ginger_beer = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.com.au/beer-and-cider/beer/ginger-beer?page=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(`More than 5 entries missing for liquorland_aus - beer-and-cider-beer-ginger-beer: ${pageNo}`, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError(`Error in liquorland_aus - beer-and-cider-beer-ginger-beer: ${err.message}`, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
