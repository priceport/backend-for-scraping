...
const tools = async (start,end,browser)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for beauty bliss - tools: "+pageNo,"scraping_missing");
        }
        ...
        try{
          await insertScrapingError("Error in beauty bliss - tools: "+err.message,"scraping_trycatch");
        }catch(err){
          console.log(err);
        }
