...
const medicines = async (start, end, browser) => {
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy medicines] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy medicines] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy medicines page ${pageNo}`);
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy medicines] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy medicines] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - medicines: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
