...
const home_fragrance_candle = async (start,end,browser)=>{
    let pageNo = start-1;
    const url = "https://www.heinemann.com.au/en/ool/perfume-cologne/home-fragrances-candles/c/cat_4040/?q=%3Arelevance&page=";
    ...
    while(true){
        ...
        const [products,missing] = await page.evaluate(() => {
            ...
            productElements.forEach(product => {
                ...
                url, 
                category:'beauty',
                source:{website_base:"https://www.heinemann.com.au/en/ool/",location:"goldcoast",tag:"duty-free"}, 
                date:Date.now(),
                last_check:Date.now(),
                mapping_ref:null,
                unit:undefined,
                subcategory:'home_fragrance_candle',
                img,
                isMultibuy
                });
        ...
        if(missing > 5) {
            await insertScrapingError("More than 5 entries missing for heinemann_goldcoast - home_fragrance_candle: "+pageNo,"scraping_missing");
        }
        ...
        pageNo+=1;
            ...
            logError(err);
            try{
                await insertScrapingError("Error in heinemann_goldcoast - home_fragrance_candle: "+err.message,"scraping_trycatch");
              }catch(err){
                  console.log(err);
              }
            await page.close();
...
module.exports = home_fragrance_candle;