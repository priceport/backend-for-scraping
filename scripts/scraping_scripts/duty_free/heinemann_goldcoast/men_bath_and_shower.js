...
const men_bath_and_shower = async (start,end,browser)=>{
    let pageNo = start-1;
    const url = "https://www.heinemann.com.au/en/ool/perfume-cologne/men-fragrances/bath-shower/c/cat_4022/?q=%3Arelevance&page=";
    ...
    while(true){
        ...
        const [products,missing] = await page.evaluate(() => {
            ...
            productElements.forEach(product => {
                ...
                mapping_ref:null,
                unit:undefined,
                subcategory:'men_bath_and_shower',
                img,
                isMultibuy
                });
        ...
        if(missing > 5) {
            await insertScrapingError("More than 5 entries missing for heinemann_goldcoast - men_bath_and_shower: "+pageNo,"scraping_missing");
        }
        ...
        pageNo+=1;
            ...
            logError(err);
            try{
                await insertScrapingError("Error in heinemann_goldcoast - men_bath_and_shower: "+err.message,"scraping_trycatch");
              }catch(err){
                  console.log(err);
              }
            await page.close();
...
module.exports = men_bath_and_shower;