...
const carbonated = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/other/mixers-non-alcoholic-drinks/carbonated?p=';
  ...
  try {
    ...
    while (true) {
        ...
        productElements.forEach(product => {
          ...
          if (!title || !price || !url) {
              ...
              brand: null, 
                ...
                promo: null,
                url,
              category: 'mixers-non-alcoholic-drinks',
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - carbonated: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - carbonated: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
