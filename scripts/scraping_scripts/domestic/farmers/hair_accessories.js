...
const hair_accessories = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/hair-care-colour/hair-accessories";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - hair_accessories: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ hair_accessories scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ hair_accessories scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - hair_accessories (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = hair_accessories;
