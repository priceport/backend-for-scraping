...
const electronics_travel = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/travel-and-services/electronics-travel?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Electronics Travel] Loading page ${pageNo}...`);
      ...
      const [products,missing] = await page.evaluate(() => {
        ...
        productElements.forEach((productLink, index) => {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "electronics_travel",
              img: imgSrc,
            });
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - electronics_travel: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - electronics_travel: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = electronics_travel;
