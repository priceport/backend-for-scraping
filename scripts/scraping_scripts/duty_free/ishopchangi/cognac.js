...
const cognac = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/wine-and-spirits/cognac?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Cognac] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - cognac: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - cognac: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
