...
const womensPerfume = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers womensPerfume scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" womensPerfume ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - womensPerfume: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers womensPerfume scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - womensPerfume: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
