        ...
        try {
            ...
            if (product.rowCount === 0) {
                ...
                product = await pool.query(
                    ...
                    [title, null, "No desc", url, img, quantity, unit, category, sub_category, "big_barrel", "domestic"]
                );
                await pool.query(
                    `INSERT INTO price (product_id, date, price, website, price_per_unit)
                    VALUES ($1, current_date, $2, $3, $4)`,
                    [product?.rows[0]?.id, price[0].price, "big_barrel", price_per_unit]
                );
                new_prices += 1;
            } else {
            ...
            // Promo insertion logic (commented out as per original code)
            // if (promo) {
            //     for (let i = 0; i < promo?.length; i++) {
            //         await pool.query(
            //             `INSERT INTO promotion_from_aelia_auckland (prod_id, text, price) 
            //             VALUES ($1, $2, $3)`,
            //             [product?.rows[0]?.id, promo[i]?.text, promo[i]?.price]
            //         );
            //     }
            // }
