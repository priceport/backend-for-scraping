...
const special = async (start,end,browser)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for big barrel - special: "+pageNo,"scraping_missing");
        }
        ...
        try{
          await insertScrapingError("Error in big barrel - special: "+err.message,"scraping_trycatch");
        }catch(err){
          console.log(err);
        }
