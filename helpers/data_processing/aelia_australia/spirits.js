...
function calculatePriceFromText(text, og_price) {
    ...
    offers.forEach(offer => {
        ...
        if (matchForXForY) {
            ...
            prices.push({ text: offer, price: aud_to_usd((price / quantity).toFixed(3)) }); // Price per item
            return;
        }
        ...
        if (matchBuyXSaveYPercent) {
            ...
            prices.push({text:offer,price:aud_to_usd(effectivePrice.toFixed(3))}); // Apply the discount to the original price
            return;
        }
    });
...
const processDataForSpirits = async (data)=>{
    ...
    while(iterator<data?.length){
        ...
        if(aud_to_usd(rawData.price.replace("$",""),"aelia auckland")=="Invalid input"){
            iterator+=1;
            continue;
        } 
        ...
        try{
            ...
            finalData.price = [{text:"",price:aud_to_usd(rawData.price.replace("$",""),"aelia auckland")}];
