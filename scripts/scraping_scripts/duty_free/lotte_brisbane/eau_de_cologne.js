...
const eau_de_cologne = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://www.lottedutyfree.com.au/collections/eau-de-cologne?page=";
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_brisbane - eau_de_cologne: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_brisbane - eau_de_cologne: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
