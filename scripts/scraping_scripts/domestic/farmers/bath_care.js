...
const bath = async (start,end,browser)=>{
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - bath: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - bath: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
