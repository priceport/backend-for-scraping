...
const fortified = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://www.aeliadutyfree.co.nz/auckland/wine/fortified.html?p=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for aelia_auckland - fortified: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in aelia_auckland - fortified: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
