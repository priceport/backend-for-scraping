...
const bath = async (start,end)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for sephora - bath: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in sephora - bath: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
