...
const medicated_skincare = async (start, end, browser) => {
  ...
  const url = 'https://www.lifepharmacy.co.nz/collections/medicated-skincare?page=';
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy medicated_skincare] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy medicated_skincare] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy medicated_skincare page ${pageNo}`);
      }
        ...
        productElements.forEach((product) => {
          ...
          if (title && price && url) {
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: 'medicated_skincare',
              img,
            });
          }
        });
      ...
      if (missing > 5) {
        await insertScrapingError(
          'More than 5 entries missing for lifepharmacy - medicated_skincare: ' + pageNo,
          'scraping_missing'
        );
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy medicated_skincare] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy medicated_skincare] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - medicated_skincare: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
...
module.exports = medicated_skincare;
