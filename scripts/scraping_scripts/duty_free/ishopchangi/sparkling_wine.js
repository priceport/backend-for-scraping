...
const sparkling_wine = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/wine-and-spirits/wines-and-champagnes/sparkling-wine?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Sparkling Wine] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - sparkling_wine: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - sparkling_wine: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
