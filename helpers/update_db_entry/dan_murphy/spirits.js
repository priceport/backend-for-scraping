...
const updateDBEntry = async (data) => {
    ...
    while (iterator < data?.length) {
        try {
            let { url, category, name, brand, price, unit, quantity, sub_category, img, promo, website ,description} = data[iterator];
            let product = await pool.query("SELECT * FROM product WHERE url = $1 AND website = $2", [url, website]);
            ...
            if (product.rowCount === 0) {
                ...
                product = await pool.query(
                    ...
                    [name, brand, description, url, img, quantity, unit, category, sub_category, website, "domestic"]
                );
                await pool.query(
                    `INSERT INTO price (product_id, date, price, website, price_per_unit)
                    VALUES ($1, current_date, $2, $3, $4)`,
                    [product?.rows[0]?.id, price[0].price, website, price_per_unit]
                );
            } else {
