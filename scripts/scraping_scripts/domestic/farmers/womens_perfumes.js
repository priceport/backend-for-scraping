...
const womens_perfumes = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/perfume/women-s-perfumes";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - womens_perfumes: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ womens_perfumes scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ womens_perfumes scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - womens_perfumes (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = womens_perfumes;
