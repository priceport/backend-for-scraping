...
const brandyCognac = async () => {
    ...
    retryCount = 0;
    let brandyPageLoaded = false;
    
    while (retryCount < maxRetries && !brandyPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/brandy-cognac', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Brandy cognac page loaded successfully");
            brandyPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading brandy cognac page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/brandy-cognac', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Brandy cognac page loaded with domcontentloaded");
                    brandyPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load brandy cognac page after all retries");
                throw new Error(`Failed to load brandy cognac page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/brandy-cognac?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
