...
const souvenir_food = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/food-and-beverages/souvenir-food?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Souvenir Food] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - souvenir_food: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - souvenir_food: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
