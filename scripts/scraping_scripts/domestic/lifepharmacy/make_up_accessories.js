...
const make_up_accessories = async (start, end, browser) => {
  ...
  const url = 'https://www.lifepharmacy.co.nz/collections/make-up-accessories?page=';
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy make_up_accessories] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy make_up_accessories] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy make_up_accessories page ${pageNo}`);
      }
        ...
        productElements.forEach((product) => {
          ...
          if (title && price && url) {
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: 'make_up_accessories',
              img,
            });
          }
        });
      ...
      if (missing > 5) {
        await insertScrapingError(
          'More than 5 entries missing for lifepharmacy - make_up_accessories: ' + pageNo,
          'scraping_missing'
        );
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy make_up_accessories] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy make_up_accessories] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - make_up_accessories: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
...
module.exports = make_up_accessories;
