...
const hair_accesories = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers hair_accesories scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" hair_accesories ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - hair_accesories: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers hair_accesories scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - hair_accesories: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
