...
const whiteWine = async () => {
    ...
    console.log("Navigating to Dan Murphy's white wine page...");
    ...
    // Then navigate to the white wine page
    retryCount = 0;
    let whiteWinePageLoaded = false;
    
    while (retryCount < maxRetries && !whiteWinePageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/white-wine/all', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("White wine page loaded successfully");
            whiteWinePageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading white wine page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/white-wine/all', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("White wine page loaded with domcontentloaded");
                    whiteWinePageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load white wine page after all retries");
                throw new Error(`Failed to load white wine page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/white-wine/all?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
