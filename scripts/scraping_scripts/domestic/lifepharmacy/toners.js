  ...
  try {
    ...
    while (pageNo <= maxPage) {
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy toners] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy toners] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
