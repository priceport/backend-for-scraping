...
const snacks_and_sweets = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/food-and-beverages/snacks-and-sweets?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Snacks & Sweets] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - snacks_and_sweets: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - snacks_and_sweets: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = snacks_and_sweets;
