...
const eye_cream = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/skincare/eye-cream";
  ...
  try {
    ...
    console.log(`✅ eye_cream scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ eye_cream scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - eye_cream (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
