...
const sleep = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/wellness-supplements/sleep";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - sleep: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ sleep scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ sleep scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - sleep (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
