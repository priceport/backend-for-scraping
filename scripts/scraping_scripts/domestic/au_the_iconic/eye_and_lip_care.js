...
const eye_and_lip_care = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-skincare-eye-lip-care/";
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - eye_and_lip_care: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - eye_and_lip_care: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = eye_and_lip_care;
