    ...
    try {
      ...
      if (product.rowCount === 0) {
        ...
        await pool.query(
          ...
          [product?.rows[0]?.id, price[0].price, "liquorland_aus", price_per_unit]
        );
        new_prices += 1;
        product_created += 1;
      } else {
        ...
        // Update last_checked timestamp and brand
        await pool.query(
          `UPDATE product 
                    SET last_checked = current_timestamp , country = $2, brand = $3
                    WHERE id = $1`,
