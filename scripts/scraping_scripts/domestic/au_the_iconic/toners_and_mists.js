...
const toners_and_mists = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-skincare-toners-mists/";
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - toners_and_mists: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - toners_and_mists: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = toners_and_mists;
