...
const medication = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/health-and-wellness/medication-and-treatment?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Medication] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - medication: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - medication: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
