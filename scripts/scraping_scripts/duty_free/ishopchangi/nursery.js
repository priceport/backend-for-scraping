...
const nursery = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/babies-and-kids/babies-and-kids-nursery?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Nursery] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - nursery: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - nursery: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
