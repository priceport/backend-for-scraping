...
const eye_cream = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers eye_cream scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" eye_cream ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - eye_cream: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers eye_cream scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - eye_cream: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
