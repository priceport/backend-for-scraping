...
const coffee_and_tea = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/food-and-beverages/food-and-beverages-coffee-and-tea?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Coffee & Tea] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - coffee_and_tea: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - coffee_and_tea: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
