...
const single_malt = async (start,end,browser)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for whisky_and_more - single_malt: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in whisky_and_more - single_malt: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
