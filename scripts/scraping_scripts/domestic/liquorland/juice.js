...
const juice = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/other/mixers-non-alcoholic-drinks/fruit-juice?p=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - juice: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - juice: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
