...
const eyes = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-make-up-eyes/";
  ...
  try {
    ...
    while (true) {
      ...
      const [products, missing] = await page.evaluate(() => {
        ...
        productElements.forEach((product) => {
          ...
          let promo; // Reserved for future use
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - eyes: " + pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - eyes: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
