...
function parseProductTitle(input) {
    ...
    logInvalid({text:input,source:"chemist warehouse"});
    return {
        title:input,
        quantity:null,
        unit:null
...
const processDataForBeauty = async (data)=>{
    ...
    while(iterator<data?.length){
        ...
        if(nzd_to_usd(rawData.price.replace("$",""),"chemist warehouse")=="Invalid input"){
            iterator+=1;
            continue;
        } 
        ...
        try{
            ...
            finalData.price = [{text:"",price:nzd_to_usd(rawData.price.replace("$",""),"chemist warehouse")}];
