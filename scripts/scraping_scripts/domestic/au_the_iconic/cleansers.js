...
const cleansers = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-skincare-cleansers/";
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - cleansers: " + pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - cleansers: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
