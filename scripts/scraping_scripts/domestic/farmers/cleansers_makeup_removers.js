...
const cleansers_makeup_removers = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/skincare/cleansers-makeup-removers";
  ...
  try {
    ...
    const formattedProducts = allProducts.map(product => {
      ...
      return {
        ...
        mapping_ref: null,
        unit: undefined,
        subcategory: 'cleansers_makeup_removers',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - cleansers_makeup_removers: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ cleansers_makeup_removers scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ cleansers_makeup_removers scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - cleansers_makeup_removers (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = cleansers_makeup_removers;
