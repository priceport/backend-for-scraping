...
const cleansers = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/skincare/cleansers-makeup-removers';
    const url = 'https://www.farmers.co.nz/beauty/skincare/cleansers-makeup-removers/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for farmers - cleansers: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in farmers - cleansers: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
