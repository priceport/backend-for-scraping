...
const pinot_noir = async (start, end, browser) => {
  let pageNo = start;
  const baseUrl = 'https://www.liquorland.co.nz/shop/wine/red/pinot-noir?FF=&InStoreOnly=false&P.StartPage=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - pinot_noir: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - pinot_noir: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
