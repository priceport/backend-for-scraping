/**
 * Farmers Face Products Scraper (Template)
...
const face_template = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers face_template scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" face ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - face_template: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers face_template scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - face_template: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
