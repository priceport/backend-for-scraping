...
const port_and_sherry = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://www.lottedutyfree.com.au/collections/port-and-sherry?page=";
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_brisbane - port_and_sherry: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_brisbane - port_and_sherry: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
