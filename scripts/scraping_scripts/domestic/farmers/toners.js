...
const toners = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/skincare/toners';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for farmers - toners: "+pageNo,"scraping_missing");
        }
        ...
        try{
          await insertScrapingError("Error in farmers - toners: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
