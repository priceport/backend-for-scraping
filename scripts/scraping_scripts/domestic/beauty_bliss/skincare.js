...
const skincare = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://beautybliss.co.nz/product-category/skin-care/?SkipCount=replace_me';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for beauty bliss - skincare: "+pageNo,"scraping_missing");
        }
        ...
        try{
          await insertScrapingError("Error in beauty bliss - skincare: "+err.message,"scraping_trycatch");
        }catch(err){
          console.log(err);
        }
