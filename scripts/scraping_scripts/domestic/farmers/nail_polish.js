...
const nail_polish = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/nail-care/nail-polish';
    const url = 'https://www.farmers.co.nz/beauty/nail-care/nail-polish/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for farmers - nail_polish: "+pageNo,"scraping_missing");
        }
        ...
        try{
          await insertScrapingError("Error in farmers - nail_polish: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
