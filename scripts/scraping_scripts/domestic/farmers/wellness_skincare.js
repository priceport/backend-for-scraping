...
const wellness_skincare = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers wellness_skincare scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" wellness_skincare ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - wellness_skincare: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers wellness_skincare scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - wellness_skincare: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = wellness_skincare;
