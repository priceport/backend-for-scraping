...
const menAftershave = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers menAftershave scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" menAftershave ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - menAftershave: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers menAftershave scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - menAftershave: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
