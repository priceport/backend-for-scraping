...
const hair = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://www.mecca.com/en-nz/haircare/?page=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for mecca - hair: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in farmers - hair: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
