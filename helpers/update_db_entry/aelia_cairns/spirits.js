...
const updateDBEntry = async (data) => {
    ...
    while (iterator < data?.length) {
        try {
            ...
            let product = await pool.query("SELECT * FROM product WHERE url = $1 AND website = $2", [url, 'aelia_cairns']);
            ...
            if (product.rowCount === 0) {
                ...
                product = await pool.query(
                    ...
                    [title, brand, "No desc", url, img, quantity, unit, category, sub_category, "aelia_cairns", "duty-free"]
                );
                await pool.query(
                    `INSERT INTO price (product_id, date, price, website, price_per_unit)
                    VALUES ($1, current_date, $2, $3, $4)`,
                    [product?.rows[0]?.id, price[0].price, "aelia_cairns", price_per_unit]
                );
            } else {
                ...
                const latestPrice = await pool.query(
                    ...
                    ORDER BY date DESC 
                    LIMIT 1`,
                    [product?.rows[0]?.id, "aelia_cairns"]
                );
            ...
            if (promo) {
                for (let i = 0; i < promo?.length; i++) {
                    await pool.query(
                        ...
                        VALUES ($1, $2, $3, $4)`,
                        [product?.rows[0]?.id, promo[i]?.text, promo[i]?.price, "aelia_cairns"]
                    );
                }
            }
