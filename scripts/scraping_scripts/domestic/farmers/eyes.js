...
const eyes = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/makeup/eyes";
  ...
  try {
    ...
    console.log(`✅ eyes scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ eyes scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - eyes (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
