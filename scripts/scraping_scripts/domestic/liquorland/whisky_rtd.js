...
const whisky_rtd = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/rtd/whisky-rtd?p=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - whisky_rtd: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - whisky_rtd: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
