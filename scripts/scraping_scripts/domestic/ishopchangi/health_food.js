...
const health_food = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/food-and-beverages/health-food?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Health Food] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - health_food: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - health_food: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
