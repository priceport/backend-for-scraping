...
const eyes = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers eyes scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" eyes ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - eyes: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers eyes scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - eyes: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
