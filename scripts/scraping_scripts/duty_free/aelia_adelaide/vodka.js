...
const vodka = async (start, end, browser) => {
    let pageNo = start;
    const url = 'https://www.aeliadutyfree.com.au/adelaide/spirits/vodka.html?p=';
    ...
    try {
        ...
        while(true) {
            ...
            if(missing > 5) {
                await insertScrapingError("More than 5 entries missing for aelia_adelaide - vodka: " + pageNo, "scraping_missing");
            }
        ...
        logError(err);
        try {
            await insertScrapingError("Error in aelia_adelaide - vodka: " + err.message, "scraping_trycatch");
        } catch(err) {
            console.log(err);
        }
        await page.close();
