...
const nz_whisky = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/spirits/whisky/nz-whisky?p=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - nz_whisky: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - nz_whisky: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
