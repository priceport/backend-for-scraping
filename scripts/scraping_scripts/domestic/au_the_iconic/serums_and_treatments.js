...
const serums_and_treatments = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-skincare-skincare-tools/";
  ...
  try {
    ...
    while (true) {
      ...
      await page.goto(url + (pageNo > 1 ? `?p=${pageNo}` : ""), {
        ...
        productElements.forEach((product) => {
          ...
          if (!title && !brand && !finalPrice && !promo && !fullUrl) {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "serums_and_treatments",
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - serums_and_treatments: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - serums_and_treatments: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = serums_and_treatments;
