...
const face_optimized = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers face_optimized scraper (pages ${start} to ${end})...`);
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - face_optimized: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers face_optimized scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - face_optimized: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
