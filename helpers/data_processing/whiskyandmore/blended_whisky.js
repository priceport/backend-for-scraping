...
function calculatePriceFromText(text, og_price) {
    ...
    offers.forEach(offer => {
        ...
        if (matchBuyXSaveYPercent) {
            ...
            prices.push({text:offer,price:effectivePrice.toFixed(2)}); // Apply the discount to the original price
            return;
        }
    });
...
function parseProductTitle(input) {
    ...
    logInvalid({text:input,source:"whisky and more"});
    return null;
}
...
//main function
const processDataForBlendedWhisky = async (data)=>{
    const output = [];
    let iterator = 0;
...
module.exports = processDataForBlendedWhisky;