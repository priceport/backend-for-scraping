        ...
        try {
            ...
            if (product.rowCount === 0) {
                ...
                product = await pool.query(
                    ...
                    [title?.slice(0, 299), brand, "No desc", url, img, quantity, unit, category, sub_category, "heinemann_goldcoast", "duty-free"]
                );
            } else {
                // console.log("old");
            ...
            const latestPrice = await pool.query(
                ...
                ORDER BY date DESC 
                LIMIT 1`,
                [product?.rows[0]?.id, "heinemann_goldcoast"]
            );
            ...
            if (latestPrice.rowCount === 0 || latestPrice.rows[0].price != price[0].price.toFixed(3)) {
                await pool.query(
                    ...
                    [product?.rows[0]?.id, price[0].price, "heinemann_goldcoast", price_per_unit]
                );
                new_prices += 1;
            }
            ...
            for (let i = 0; i < promo?.length; i++) {
                await pool.query(
                    ...
                    VALUES ($1, $2, $3, $4)`,
                    [product?.rows[0]?.id, promo[i]?.text, promo[i]?.price, "heinemann_goldcoast"]
                );
                ...
                //     //     VALUES ($1, $2, $3, $4, $5)`,
                //     //     [product?.rows[0]?.id, promo[i], p, "heinemann_goldcoast", "duty-free"]
                //     // );
                ...
                //     //     `INSERT INTO promotion (product_id, text, price, website) 
                //     //     VALUES ($1, $2, $3, $4)`,
                //     //     [product?.rows[0]?.id, promo[i], p, "heinemann_goldcoast"]
                //     // );
                // }
            }
