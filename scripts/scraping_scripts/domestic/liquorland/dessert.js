...
const dessert = async (start, end, browser) => {
  let pageNo = start;
  const baseUrl = 'https://www.liquorland.co.nz/shop/wine/white/dessert?FF=&InStoreOnly=false&P.StartPage=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - dessert: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - dessert: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
