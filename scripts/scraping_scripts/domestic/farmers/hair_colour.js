...
const hair_colour = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/hair-care-colour/hair-colour";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - hair_colour: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ hair_colour scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ hair_colour scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - hair_colour (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
