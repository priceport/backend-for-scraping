...
const lips = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://www.lottedutyfree.com.au/collections/lips?page=";
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_brisbane - lips: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_brisbane - lips: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
