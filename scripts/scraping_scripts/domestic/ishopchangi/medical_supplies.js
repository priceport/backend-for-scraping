...
const medical_supplies = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/health-and-wellness/first-aid-and-medical-supplies?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Medical Supplies] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - medical_supplies: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - medical_supplies: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
