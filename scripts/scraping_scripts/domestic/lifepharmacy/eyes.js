...
const eyes = async (start, end, browser) => {
  ...
  const url = 'https://www.lifepharmacy.co.nz/collections/eyes?page=';
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy eyes] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy eyes] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy eyes page ${pageNo}`);
      }
      ...
      if (missing > 5) {
        await insertScrapingError(
          'More than 5 entries missing for lifepharmacy - eyes: ' + pageNo,
          'scraping_missing'
        );
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy eyes] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy eyes] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - eyes: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
