...
const bags_and_tools = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-make-up-bags-tools/";
  ...
  try {
    ...
    while (true) {
      ...
      const [products, missing] = await page.evaluate(() => {
        ...
        productElements.forEach((product) => {
          ...
          let promo; // placeholder for future promo extraction if needed
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - bags_and_tools: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - bags_and_tools: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
