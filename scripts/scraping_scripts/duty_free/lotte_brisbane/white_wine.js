...
const white_wine = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://www.lottedutyfree.com.au/collections/white-wine?page=";
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_brisbane - white_wine: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_brisbane - white_wine: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
