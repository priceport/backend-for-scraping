...
const menAftershave = async (start, end, browser) => {
  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/fragrance/mens-aftershave";
  ...
  try {
    ...
    try {
      await insertScrapingError(
        `Error in farmers - menAftershave (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
