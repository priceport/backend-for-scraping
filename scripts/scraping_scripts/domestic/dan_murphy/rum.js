...
const rum = async () => {
    ...
    console.log("Navigating to Dan Murphy's rum page...");
    ...
    // Then navigate to the rum page
    retryCount = 0;
    let rumPageLoaded = false;
    
    while (retryCount < maxRetries && !rumPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/rum', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Rum page loaded successfully");
            rumPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading rum page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/rum', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Rum page loaded with domcontentloaded");
                    rumPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load rum page after all retries");
                throw new Error(`Failed to load rum page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/rum?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
