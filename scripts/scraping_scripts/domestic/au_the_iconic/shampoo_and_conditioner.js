...
const shampoo_and_conditioner = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-hair-shampoo-conditioner/";
  ...
  try {
    ...
    while (true) {
        ...
        productElements.forEach((product) => {
          ...
          if (!title && !brand && !finalPrice && !promo && !fullUrl) {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "shampoo_and_conditioner",
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - shampoo_and_conditioner: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - shampoo_and_conditioner: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = shampoo_and_conditioner;
