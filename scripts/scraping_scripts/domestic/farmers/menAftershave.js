...
const menAfterShave = async (start,end,browser)=>{
    let pageNo = start;
    const default_url = 'https://www.farmers.co.nz/beauty/perfume/men-s-aftershaves-cologne';
    const url = 'https://www.farmers.co.nz/beauty/perfume/men-s-aftershaves-cologne/Page-replace_me-SortingAttribute-SortBy-asc';
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - menAfterShave: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - menAfterShave: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
