...
const wine = async (start,end,browser)=>{
    ...
    let pageNo = start;
    const url = 'https://www.whiskyandmore.co.nz/collections/wine?page=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for whisky_and_more - wine: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in whisky_and_more - wine: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
