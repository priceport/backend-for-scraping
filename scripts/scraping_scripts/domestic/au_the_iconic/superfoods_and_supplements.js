...
const superfoods_and_supplements = async (start, end, browser) => {
  let pageNo = start;
  const url =
    "https://www.theiconic.com.au/beauty-wellness-superfoods-supplements/";
  ...
  try {
    ...
    while (true) {
        ...
        productElements.forEach((product) => {
          ...
          if (!title && !brand && !finalPrice && !promo && !fullUrl) {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "superfoods_and_supplements",
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - superfoods_and_supplements: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - superfoods_and_supplements: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = superfoods_and_supplements;
