...
const spirits = async (start,end,browser)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for aelia_auckland - spirits: "+pageNo,"scraping_missing");
        }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in aelia_auckland - spirits: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
