...
const updateDBEntry = async (data) => {
  ...
  while (iterator < data?.length) {
    try {
      let { url, category, title, price, unit, quantity, sub_category, img } =
        data[iterator];
      ...
      let product = await pool.query(
        ...
        [url, "au_chemist_warehouse"]
      );

      if (product.rowCount === 0) {
        // If no product exists, create one
        product = await pool.query(
          `INSERT INTO product (title, description, url, image_url, qty, unit, category, sub_category, website, tag, country)
        ...
        const latestPrice = await pool.query(
          `SELECT price 
                    ...
                    ORDER BY date DESC 
                    LIMIT 1`,
          [product?.rows[0]?.id, "au_chemist_warehouse"]
        );
        ...
        await pool.query(
          `UPDATE product 
                    SET last_checked = current_timestamp, country = $2                    WHERE id = $1`,
