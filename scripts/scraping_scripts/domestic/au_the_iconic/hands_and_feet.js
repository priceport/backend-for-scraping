...
const hands_and_feet = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-body-hands-feet/";
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - hands_and_feet: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - hands_and_feet: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
