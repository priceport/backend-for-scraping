...
const blushers_bronzers = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/makeup/blushers-bronzers";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - blushers_bronzers: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ blushers_bronzers scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ blushers_bronzers scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - blushers_bronzers (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = blushers_bronzers;
