...
const skincare_gift_sets = async (start, end, browser) => {
  ...
  const url = 'https://www.lifepharmacy.co.nz/collections/skincare-gift-sets?page=';
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy skincare_gift_sets] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy skincare_gift_sets] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy skincare_gift_sets page ${pageNo}`);
      }
        ...
        productElements.forEach((product) => {
          ...
          if (title && price && url) {
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: 'skincare_gift_sets',
              img,
            });
          }
        });
      ...
      if (missing > 5) {
        await insertScrapingError(
          'More than 5 entries missing for lifepharmacy - skincare_gift_sets: ' + pageNo,
          'scraping_missing'
        );
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy skincare_gift_sets] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy skincare_gift_sets] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - skincare_gift_sets: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
...
module.exports = skincare_gift_sets;
