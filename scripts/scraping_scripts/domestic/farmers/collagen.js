/**
 * Farmers Collagen Products Scraper
...
const collagen = async (start, end) => {
  const allProducts = [];
  ...
  try {
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" collagen ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - collagen: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers collagen scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - collagen: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
