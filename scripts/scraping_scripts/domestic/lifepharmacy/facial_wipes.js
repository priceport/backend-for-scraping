...
const facial_wipes = async (start, end, browser) => {
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy facial_wipes] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy facial_wipes] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy facial_wipes page ${pageNo}`);
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy facial_wipes] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy facial_wipes] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - facial_wipes: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
