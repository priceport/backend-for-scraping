...
const brandy_and_cognac = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.com.au/spirits/brandy-and-cognac?page=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(`More than 5 entries missing for liquorland_aus - brandy-and-cognac: ${pageNo}`, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError(`Error in liquorland_aus - brandy-and-cognac: ${err.message}`, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
...
module.exports = brandy_and_cognac;