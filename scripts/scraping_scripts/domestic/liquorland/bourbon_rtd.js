...
const bourbon_rtd = async (start, end, browser) => {
  let pageNo = start;
  const url = 'https://www.liquorland.co.nz/rtd/bourbon-rtd?p=';
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - bourbon_rtd: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - bourbon_rtd: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
