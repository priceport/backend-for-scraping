...
const masks_peels = async (start, end, browser) => {
  ...
  const url = 'https://www.lifepharmacy.co.nz/collections/masks-peels?page=';
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy masks_peels] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy masks_peels] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy masks_peels page ${pageNo}`);
      }
      ...
      if (missing > 5) {
        await insertScrapingError(
          'More than 5 entries missing for lifepharmacy - masks_peels: ' + pageNo,
          'scraping_missing'
        );
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy masks_peels] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy masks_peels] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - masks_peels: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
