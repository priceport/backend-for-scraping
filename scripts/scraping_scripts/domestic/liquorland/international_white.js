...
const international_white = async (start, end, browser) => {
  let pageNo = start;
  const baseUrl = 'https://www.liquorland.co.nz/shop/wine/white/international-white?FF=&InStoreOnly=false&P.StartPage=';
  ...
  try {
    ...
    while (true) {
          ...
          try {
            ...
            if (title && price) {
              ...
              productList.push({
                ...
                mapping_ref: null,
                unit: undefined,
                subcategory: 'international_white',
                img
              });
            }
      ...
      if (missing > 5) {
        await insertScrapingError("More than 5 entries missing for liquorland - international_white: " + pageNo, "scraping_missing");
      }
    ...
    logError(err);
    try {
      await insertScrapingError("Error in liquorland - international_white: " + err.message, "scraping_trycatch");
    } catch (err) {
      console.log(err);
    }
    await page.close();
...
module.exports = international_white;
