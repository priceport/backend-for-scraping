...
const lips = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers lips scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" lips ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    console.log(`Farmers lips scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - lips: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
