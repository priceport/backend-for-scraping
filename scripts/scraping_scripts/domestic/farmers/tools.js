/**
 * Farmers Tools Products Scraper
...
const tools = async (start, end) => {
  ...
  try {
    console.log(`Starting Farmers tools scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" tools ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - tools: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers tools scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - tools: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
