...
const liqueurs = async () => {
    ...
    console.log("Navigating to Dan Murphy's liqueurs page...");
    ...
    // Then navigate to the liqueurs page
    retryCount = 0;
    let liqueursPageLoaded = false;
    
    while (retryCount < maxRetries && !liqueursPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/liqueurs', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Liqueurs page loaded successfully");
            liqueursPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading liqueurs page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/liqueurs', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Liqueurs page loaded with domcontentloaded");
                    liqueursPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load liqueurs page after all retries");
                throw new Error(`Failed to load liqueurs page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/liqueurs?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
