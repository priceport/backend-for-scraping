...
const beauty = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://lottedutyfree.com.au/collections/beauty?page=";
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_brisbane - beauty: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_brisbane - beauty: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
