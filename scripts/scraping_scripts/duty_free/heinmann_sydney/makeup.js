...
const makeup = async (start,end,browser)=>{
    let pageNo = start-1;
    const url = "https://www.heinemann.com.au/en/sydt1/products/make-up/c/auscat_3000/?q=%3Arelevance&page=";
    ...
    while(true){
        ...
        if(missing > 5) {
            await insertScrapingError("More than 5 entries missing for heinemann_sydney - makeup: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
            await insertScrapingError("Error in heinemann_sydney - makeup: "+err.message,"scraping_trycatch");
          }catch(err){
              console.log(err);
          }
        await page.close();
