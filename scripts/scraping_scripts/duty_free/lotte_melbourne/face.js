...
const face = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://melbourne.lottedutyfree.com.au/collections/face?page=";
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const apiData = await fetchImageFromAPI(product.url, 'face');
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_melbourne - face: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_melbourne - face: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
