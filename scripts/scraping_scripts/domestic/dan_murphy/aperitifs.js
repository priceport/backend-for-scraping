...
const aperitifs = async () => {
    ...
    console.log("Navigating to Dan Murphy's aperitifs page...");
    ...
    // Then navigate to the aperitifs page
    retryCount = 0;
    let aperitifsPageLoaded = false;
    
    while (retryCount < maxRetries && !aperitifsPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/aperitifs', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Aperitifs page loaded successfully");
            aperitifsPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading aperitifs page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/aperitifs', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Aperitifs page loaded with domcontentloaded");
                    aperitifsPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load aperitifs page after all retries");
                throw new Error(`Failed to load aperitifs page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/aperitifs?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
