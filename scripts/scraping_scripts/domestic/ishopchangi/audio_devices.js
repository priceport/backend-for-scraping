...
const audio_devices = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/electronics/audio-devices?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Audio Devices] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - audio_devices: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - audio_devices: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
