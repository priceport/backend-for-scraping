...
const collagens = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/wellness-supplements/collagens";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - collagens: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ collagens scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ collagens scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - collagens (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
