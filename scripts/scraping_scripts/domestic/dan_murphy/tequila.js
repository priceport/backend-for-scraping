...
const tequila = async () => {
    ...
    retryCount = 0;
    let tequilaPageLoaded = false;
    
    while (retryCount < maxRetries && !tequilaPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/tequila', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Tequila page loaded successfully");
            tequilaPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading tequila page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/tequila', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Tequila page loaded with domcontentloaded");
                    tequilaPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load tequila page after all retries");
                throw new Error(`Failed to load tequila page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/tequila?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
