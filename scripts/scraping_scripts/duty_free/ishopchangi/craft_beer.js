...
const craft_beer = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/wine-and-spirits/beers/craft-beer?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Craft Beer] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - craft_beer: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - craft_beer: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
