...
const foot_care = async (start,end,browser)=>{
    ...
    try{
        ...
        while(true){
            ...
            if(missing > 5) {
              await insertScrapingError("More than 5 entries missing for farmers - foot_care: "+pageNo,"scraping_missing");
            }
        ...
        try{
          await insertScrapingError("Error in farmers - foot_care: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
