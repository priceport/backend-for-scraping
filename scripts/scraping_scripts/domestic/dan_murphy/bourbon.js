...
const bourbon = async () => {
    ...
    console.log("Navigating to Dan Murphy's bourbon page...");
    ...
    // Then navigate to the bourbon page
    retryCount = 0;
    let bourbonPageLoaded = false;
    
    while (retryCount < maxRetries && !bourbonPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/whisky/american-whiskey/bourbon', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Bourbon page loaded successfully");
            bourbonPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading bourbon page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/whisky/american-whiskey/bourbon', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Bourbon page loaded with domcontentloaded");
                    bourbonPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load bourbon page after all retries");
                throw new Error(`Failed to load bourbon page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/whisky/american-whiskey/bourbon?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
