...
const moisturizers = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers moisturizers scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" moisturizers ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - moisturizers: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers moisturizers scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - moisturizers: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
