...
const body_care = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/bath-body-care/body-care";
  ...
  try {
    ...
    console.log(`✅ body_care scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ body_care scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - body_care (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
