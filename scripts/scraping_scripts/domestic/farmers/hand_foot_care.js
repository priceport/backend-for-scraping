...
const hand_foot_care = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/bath-body-care/hand-foot-care";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - hand_foot_care: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ hand_foot_care scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ hand_foot_care scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - hand_foot_care (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
