...
const personal_fragrance = async (start, end, browser) => {
  let pageNo = start;
  const url =
    "https://www.theiconic.com.au/beauty-fragrance-personal-fragrance/";
  ...
  try {
    ...
    while (true) {
        ...
        productElements.forEach((product) => {
          ...
          if (!title && !brand && !finalPrice && !promo && !fullUrl) {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "personal_fragrance",
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - personal_fragrance: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - personal_fragrance: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
...
module.exports = personal_fragrance;
