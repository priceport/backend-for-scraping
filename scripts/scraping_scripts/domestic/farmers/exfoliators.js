...
const exfoliators = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers exfoliators scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" exfoliators ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - exfoliators: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers exfoliators scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - exfoliators: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
