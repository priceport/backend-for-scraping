...
const single_malt = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://melbourne.lottedutyfree.com.au/collections/single-malt?page=";
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_melbourne - single_malt: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
