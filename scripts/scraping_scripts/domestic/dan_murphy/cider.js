...
const cider = async () => {
    ...
    console.log("Navigating to Dan Murphy's cider page...");
    ...
    // Then navigate to the cider page
    retryCount = 0;
    let ciderPageLoaded = false;
    
    while (retryCount < maxRetries && !ciderPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/cider/all', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Cider page loaded successfully");
            ciderPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading cider page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/cider/all', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Cider page loaded with domcontentloaded");
                    ciderPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load cider page after all retries");
                throw new Error(`Failed to load cider page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/cider/all?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
