...
const food_staples = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/food-and-beverages/food-staples-and-cooking-essentials?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Food Staples] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - food_staples: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - food_staples: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
