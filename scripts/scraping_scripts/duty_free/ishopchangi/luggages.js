...
const luggages = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/travel-and-services/luggages?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Luggages] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - luggages: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - luggages: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
