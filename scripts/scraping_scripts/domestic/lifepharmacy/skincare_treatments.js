...
const skincare_treatments = async (start, end, browser) => {
  ...
  try {
    ...
    while (pageNo <= maxPage) {
      console.log(`[LifePharmacy skincare_treatments] Loading page ${pageNo}...`);
      ...
      if (!loaded) {
        ...
        if (gotoOk && allProducts.length > 0) {
          console.log(
            `[LifePharmacy skincare_treatments] No product titles on page ${pageNo}; finished (${allProducts.length} total).`
          );
          await page.close();
          return allProducts;
        }
        throw navErr || new Error(`Failed to load lifepharmacy skincare_treatments page ${pageNo}`);
      }
      ...
      if (products.length === 0) {
        console.log(
          `[LifePharmacy skincare_treatments] No products on page ${pageNo}; finished (${allProducts.length} total).`
        );
        await page.close();
        return allProducts;
      }
    ...
    console.log(
      `[LifePharmacy skincare_treatments] Reached safety max page ${maxPage}; finished (${allProducts.length} total).`
    );
    await page.close();
    return allProducts;
  } catch (err) {
    logError(err);
    await insertScrapingError(
      'Error in lifepharmacy - skincare_treatments: ' + err.message,
      'scraping_trycatch'
    );
    try {
      await page.close();
