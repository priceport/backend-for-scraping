...
const bath_and_shower = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.theiconic.com.au/beauty-body-bath-shower/";
  ...
  try {
    ...
    while (true) {
      ...
      if (missing > 5) {
        await insertScrapingError(
          "More than 5 entries missing for the_iconic - bath_and_shower: " +
            pageNo,
          "scraping_missing"
        );
      }
    ...
    try {
      await insertScrapingError(
        "Error in the_iconic - bath_and_shower: " + err.message,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
