...
const nail_care_tools = async (start, end, browser, sharedPage) => {

  const allProducts = [];
  let missingTotal = 0;
  const baseUrl = "https://www.farmers.co.nz/beauty/nail-care/nail-care-tools";
  ...
  try {
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - nail_care_tools: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`✅ nail_care_tools scraping completed: ${formattedProducts.length} products`);
    console.log(`✅ nail_care_tools scraping completed: ${formattedProducts.length} products`);
    ...
    try {
      await insertScrapingError(
        `Error in farmers - nail_care_tools (Bright Data Browser): ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      // Ignore
...
module.exports = nail_care_tools;
