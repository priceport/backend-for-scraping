...
const body = async (start,end,browser)=>{
    let pageNo = start;
    const url = 'https://www.mecca.com/en-nz/body/?page=';
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for mecca - body: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in mecca - body: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
