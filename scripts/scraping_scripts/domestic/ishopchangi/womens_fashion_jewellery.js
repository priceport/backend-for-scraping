...
const womens_fashion_jewellery = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/womens-fashion/womens-fashion-jewellery?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Women's Fashion Jewellery] Loading page ${pageNo}...`);
      ...
      const [products,missing] = await page.evaluate(() => {
        ...
        productElements.forEach((productLink, index) => {
            ...
            productList.push({
              ...
              mapping_ref: null,
              unit: undefined,
              subcategory: "womens_fashion_jewellery",
              img: imgSrc,
            });
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - womens_fashion_jewellery: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - womens_fashion_jewellery: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = womens_fashion_jewellery;
