...
const baijiu = async () => {
    ...
    console.log("Navigating to Dan Murphy's baijiu page...");
    ...
    // Then navigate to the baijiu page
    retryCount = 0;
    let baijiuPageLoaded = false;
    
    while (retryCount < maxRetries && !baijiuPageLoaded) {
        try {
            await page.goto('https://www.danmurphys.com.au/spirits/baijiu', { waitUntil: 'networkidle2', timeout: 90000 });
            console.log("Baijiu page loaded successfully");
            baijiuPageLoaded = true;
        } catch (error) {
            retryCount++;
            console.log(`Error loading baijiu page (attempt ${retryCount}/${maxRetries}):`, error.message);
            if (retryCount < maxRetries) {
                // Try with a more lenient wait condition
                try {
                    await page.goto('https://www.danmurphys.com.au/spirits/baijiu', { waitUntil: 'domcontentloaded', timeout: 90000 });
                    console.log("Baijiu page loaded with domcontentloaded");
                    baijiuPageLoaded = true;
                } catch (retryError) {
                ...
                console.log("Failed to load baijiu page after all retries");
                throw new Error(`Failed to load baijiu page after ${maxRetries} attempts: ${error.message}`);
            }
        }
    }
    ...
    if (pageContent.hasNoProducts || pageContent.hasLoading) {
        ...
        await page.goto('https://www.danmurphys.com.au/spirits/baijiu?page=1&sortBy=relevance', { waitUntil: 'networkidle2', timeout: 90000 });
