...
const international_whisky = async (start,end,browser)=>{
    ...
    while(true){
        ...
        if(missing > 5) {
          await insertScrapingError("More than 5 entries missing for whisky_and_more - international_whisky: "+pageNo,"scraping_missing");
        }
        ...
        logError(err);
        try{
          await insertScrapingError("Error in whisky_and_more - international_whisky: "+err.message,"scraping_trycatch");
        }catch(err){
            console.log(err);
        }
        await page.close();
