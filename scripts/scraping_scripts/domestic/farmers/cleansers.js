/**
 * Farmers Cleansers Products Scraper
...
const cleansers = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers cleansers scraper (pages ${start} to ${end})...`);
    ...
    const output = execSync(`${pythonExec} "${pythonScriptPath}" cleansers ${start} ${end}`, {
      encoding: 'utf-8',
    ...
    if (missingCount > 5) {
      await insertScrapingError(
        `More than 5 entries missing for farmers - cleansers: ${missingCount} products with missing data`,
        "scraping_missing"
      );
    }
    
    console.log(`Farmers cleansers scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - cleansers: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
