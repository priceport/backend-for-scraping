...
const hair_care = async (start, end, browser) => {
  let pageNo = start;
  const url = "https://www.ishopchangi.com/en/category/beauty/hair-care?page=";
  ...
  try {
    ...
    while (true) {
      console.log(`[Hair Care] Loading page ${pageNo}...`);
      ...
      if(missing > 5) {
        await insertScrapingError("More than 5 entries missing for ishopchangi - hair_care: "+pageNo,"scraping_missing");
      }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in ishopchangi - hair_care: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
