...
const eau_de_parfum = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://melbourne.lottedutyfree.com.au/collections/eau-de-parfum?page=";
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const apiData = await fetchImageFromAPI(product.url, 'eau-de-parfum');
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_melbourne - eau-de-parfum: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_melbourne - eau_de_parfum: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
