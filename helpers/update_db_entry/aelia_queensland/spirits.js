        ...
        try {
            ...
            if (product.rowCount === 0) {
                ...
                product = await pool.query(
                    ...
                    [title, brand, "No desc", url, img, quantity, unit, category, "aelia_queenstown", "duty-free"]
                );
                await pool.query(
                    `INSERT INTO price (product_id, date, price, website, price_per_unit)
                    VALUES ($1, current_date, $2, $3, $4)`,
                    [product?.rows[0]?.id, price[0].price, "aelia_queenstown", price_per_unit]
                );
                new_prices += 1;
            } else {
            ...
            if (promo) {
                for (let i = 0; i < promo?.length; i++) {
                    ...
                    await pool.query(
                        ...
                        VALUES ($1, $2, $3, $4)`,
                        [product?.rows[0]?.id, promo[i]?.text, promo[i]?.price, "aelia_queenstown"]
                    );
                }
            }
