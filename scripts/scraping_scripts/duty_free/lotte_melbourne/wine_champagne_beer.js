...
const wine_champagne_beer = async (start,end,browser)=>{
    let pageNo = start;
    const url = "https://melbourne.lottedutyfree.com.au/collections/wine-champagne-beer?page=";
        ...
        const productElements = document.querySelectorAll('.sf__col-item');
          ...
          if(!title&&!brand&&!price&&!promo&&!url){}
            ...
            productList.push({ 
              ...
              mapping_ref:null,
              unit:undefined,
              subcategory:'wine_champagne_beer',
              img: null,
              promo2
            });
          });
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      const apiData = await fetchImageFromAPI(product.url, 'wine-champagne-beer');
    ...
    if(missing > 5) {
      await insertScrapingError("More than 5 entries missing for lotte_melbourne - wine-champagne-beer: "+pageNo,"scraping_missing");
    }
    ...
    logError(err);
    try{
      await insertScrapingError("Error in lotte_melbourne - wine_champagne_beer: "+err.message,"scraping_trycatch");
    }catch(err){
        console.log(err);
    }
    await page.close();
...
module.exports = wine_champagne_beer;