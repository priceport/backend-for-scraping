...
const face = async (start, end) => {
  const allProducts = [];
  
  try {
    console.log(`Starting Farmers face scraper (pages ${start} to ${end})...`);
    ...
    console.log(`Farmers face scraping completed: ${allProducts.length} products`);
    return allProducts;
    ...
    try {
      await insertScrapingError(
        `Error in farmers - face: ${err.message}`,
        "scraping_trycatch"
      );
    } catch (err) {
      console.log(err);
